...
import { loginUser, fetchMe, fetchAlerts, assignAlert, getNextAlert } from "./api";
import "./App.css";
...
export default function App() {
  ...
  async function handleAssign(dailyAlertId) {
    setError("");
    try {
      await assignAlert(token, dailyAlertId);
    } catch (e) {
      setError(e.message);
    }
    await loadAlerts();
    ...
    <div className={`shell ${sidebarCollapsed ? "collapsed" : ""}`}>
      ...
      <main className="main-content">
        ...
        {filteredAlerts.length === 0 ? (
          ...
          <AlertTable alerts={filteredAlerts} onAssign={handleAssign} />
        )}
      </main>
    </div>
  );
